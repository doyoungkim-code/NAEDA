...
@RestController
@RequestMapping("/api/v1/identity/resident-id")
@RequiredArgsConstructor
@Tag(name = "주민등록증 OCR", description = "주민등록증 OCR 기반 본인 확인 API")
public class ResidentIdVerifyController {

    private final ResidentIdVerifyService residentIdVerifyService;

    @PostMapping(value = "/extract", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @Operation(summary = "주민등록증 OCR 추출", description = "주민등록증 이미지에서 이름과 주민등록번호 일부를 추출해 확인 화면에 사용할 값을 반환합니다.")
    @ApiResponses({
    ...
    @Operation(summary = "주민등록증 OCR 확인", description = "사용자가 확인 또는 수정한 이름과 주민등록번호 일부를 로그인 사용자 정보와 비교합니다.")
    @ApiResponses({
