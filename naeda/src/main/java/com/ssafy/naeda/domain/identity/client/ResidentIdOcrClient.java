...
public class ResidentIdOcrClient {
    ...
    public ResidentIdOcrResponse extractResidentId(MultipartFile image) {
        ...
        try {
            ...
            ResidentIdOcrResponse response = aiRestClient.post()
                    .uri("/internal/v1/ocr/resident-id/extract")
                    ...
                    .onStatus(status -> status.isError(), (req, res) -> {
                        try {
                            ...
                            throw new SsafyApiException("OCR_UNAVAILABLE", "주민등록증 OCR 서비스에 접근할 수 없습니다.");
                        }
                    })
            ...
            if (response == null || response.getName() == null || response.getResidentFront6() == null
                ...
                throw new BadRequestException("주민등록증 OCR 결과가 올바르지 않습니다.");
            }
            return response;
            ...
            log.error("주민등록증 OCR 호출 중 예외 발생", e);
            throw new SsafyApiException("OCR_UNAVAILABLE", "주민등록증 OCR 서비스에 접근할 수 없습니다.");
        } finally {
            if (imageBytes != null) {
    ...
    private RuntimeException mapAiError(String code, String message) {
        return switch (code) {
            case "OCR_EXTRACTION_FAILED" -> new BadRequestException("주민등록증 정보를 인식하지 못했습니다.");
            case "EMPTY_IMAGE", "INVALID_IMAGE", "UNSUPPORTED_IMAGE_TYPE", "IMAGE_TOO_LARGE" ->
                    new BadRequestException(message != null ? message : "주민등록증 이미지가 올바르지 않습니다.");
            case "OCR_UNAVAILABLE", "AI_UNAVAILABLE", "AI_TIMEOUT" ->
                    new SsafyApiException(code, message != null ? message : "주민등록증 OCR 서비스를 사용할 수 없습니다.");
            default -> new SsafyApiException("OCR_UNAVAILABLE", "주민등록증 OCR 서비스를 사용할 수 없습니다.");
        };
    }
}
